...
public class RobotContainer {
  ...
  private final CommandXboxController m_codriverCtrl =
      new CommandXboxController(C.OI.codriverPort);
  private final CommandPS4Controller m_driverCtrl =
      new CommandPS4Controller(C.OI.driverPortLeft);
  ...
  public RobotContainer() {
    ...
    m_Drivetrain.setDefaultCommand(
        new RunCommand(()
                           -> m_Drivetrain.Drive(m_driverCtrl.getRightX(),
                                                 m_driverCtrl.getLeftY()),
                       m_Drivetrain));

    m_Superstructure.setDefaultCommand(new RunCommand(
        ()
            -> m_Superstructure.updatejointoffsets(m_codriverCtrl.getRightY(),
                                                   m_codriverCtrl.getLeftY()),
        m_Superstructure));
  }
  ...
  private void configureButtonBindings() {
    ...
    e.onTrue(
        new InstantCommand(m_Superstructure::buttonGround, m_Superstructure));
    final Trigger s = m_codriverCtrl.x();
    s.whileTrue(
        new InstantCommand(m_Superstructure::buttonHome, m_Superstructure));
    final Trigger tcube = m_codriverCtrl.rightTrigger();
    tcube.whileTrue(new InstantCommand(m_Superstructure::buttonTopScoreCube,
                                       m_Superstructure));
    final Trigger MidScoreCube = m_codriverCtrl.rightBumper();
    MidScoreCube.whileTrue(new InstantCommand(
        m_Superstructure::buttonMidScoreCube, m_Superstructure));
    final Trigger tcone = m_codriverCtrl.leftTrigger();
    tcone.whileTrue(new InstantCommand(m_Superstructure::buttonTopScoreCone,
                                       m_Superstructure));
    final Trigger mcone = m_codriverCtrl.leftBumper();
    mcone.whileTrue(new InstantCommand(m_Superstructure::buttonMidScoreCone,
                                       m_Superstructure));
    ...
    upShoulder.whileTrue(new
    InstantCommand(m_Superstructure::moveShoulderSlowUp, m_Superstructure));
    upShoulder.onFalse(new InstantCommand(m_Superstructure::stopShoulder,
    m_Superstructure)); final Trigger downShoulder =
    m_codriverCtrl.rightTrigger(); downShoulder.whileTrue(new
    InstantCommand(m_Superstructure::moveShoulderSlowDown, m_Superstructure));
    downShoulder.onFalse(new InstantCommand(m_Superstructure::stopShoulder,
    m_Superstructure)); final Trigger upElbow = m_codriverCtrl.leftBumper();
    upElbow.whileTrue(new InstantCommand(m_Superstructure::moveElbowSlowUp,
    m_Superstructure)); upElbow.onFalse(new
    InstantCommand(m_Superstructure::stopElbow, m_Superstructure)); final
    Trigger downElbow = m_codriverCtrl.leftTrigger(); downElbow.whileTrue(new
    InstantCommand(m_Superstructure::moveElbowSlowDown, m_Superstructure));
    downElbow.onFalse(new InstantCommand(m_Superstructure::stopElbow,
    m_Superstructure)); final Trigger stopArmMotors = m_codriverCtrl.a();
    stopArmMotors.onTrue(new InstantCommand(m_Superstructure::stopJoints,
    m_Superstructure));*/

    final Trigger toggleIntake = m_codriverCtrl.y();
    toggleIntake.onTrue(
        new InstantCommand(m_Superstructure::toggleIntakeIn, m_Superstructure));
    final Trigger toggleIntakeOut = m_codriverCtrl.a();
    toggleIntakeOut.onTrue(new InstantCommand(m_Superstructure::toggleIntakeOut,
                                              m_Superstructure));
  }
